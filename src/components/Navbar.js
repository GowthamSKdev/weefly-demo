...
const Navbar = () => {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  const [isPageLoaded, setIsPageLoaded] = useState(false); // ✅ New for animation on load
  ...
  useEffect(() => {
    ...
    }, 100); // slight delay for better smoothness
  ...
  return (
    <>
      <div
        className={`fixed w-full z-50 top-0 px-10 xl:px-20 text-white font-sans transition-all duration-1000 ease-in-out ${
          isScrolled ? "backdrop-blur-lg bg-black/50" : ""
        } ${isPageLoaded ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"}`}
      >
        <div
          className={`flex justify-between items-center ${
            isScrolled ? "h-20" : "h-30"
